...
Open the chat input bar, tap the **Extras** popover, and choose **Auto Retry settings**. Options are grouped by what they do. Simple on/off switches are up top; the groups marked **Advanced** are collapsed by default, so tap one of those headers to reveal its options. Each setting has a **?** next to its name that shows a short description: hover it on a computer, tap it on a phone. The description appears just below that setting, floating over the panel rather than opening inside the list, so reading one never pushes the options out of place and never covers the setting you are asking about. It is measured from the row itself, so it stays put whatever text or UI size you have set. Only one is ever open. Tap the description, tap the **?** again, tap anywhere else, scroll, or press Esc to dismiss it.
