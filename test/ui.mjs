...
// ---- holding the button shows the hold building ----
// A press has to be held for half a second before the menu opens, and for that
// half second the button used to sit there doing nothing, so a hold that had
// registered looked exactly like one that had not. The ring is on a
// pseudo-element outside the button, so the button itself still does not move.
console.log("\nholding the floating button");
{
  ...
  const out = await page.evaluate(async () => {
    ...
    await wait(60);
    const b = host.querySelector("button");
    const ringing = () => b.hasAttribute("data-ar-hold");
    const drawn = () => {
      const cs = getComputedStyle(b, "::after");
      return { anim: cs.animationName, dur: cs.animationDuration };
    };
    const res = { atRest: ringing(), restDrawn: drawn() };
    // A press that is let go before the menu would open.
    b.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, clientX: 80, clientY: 80 }));
    await wait(80);
    res.whileHeld = ringing();
    res.heldDrawn = drawn();
    res.moved = b.style.transform || "";
    b.dispatchEvent(new PointerEvent("pointerup", { bubbles: true }));
    await wait(40);
    res.afterLetGo = ringing();
    res.menusAfterShortPress = window.__menus.length;
    // And a full hold: the ring goes when the menu arrives.
    b.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, clientX: 80, clientY: 80 }));
    await wait(700);
    res.afterMenuOpened = ringing();
    res.menus = window.__menus.length;
    b.dispatchEvent(new PointerEvent("pointerup", { bubbles: true }));
    // A drag is not a hold, and its ring goes with it.
    b.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, clientX: 80, clientY: 80 }));
    await wait(60);
    document.dispatchEvent(new PointerEvent("pointermove", { bubbles: true, clientX: 160, clientY: 160 }));
    await wait(40);
    res.afterDrag = ringing();
    b.dispatchEvent(new PointerEvent("pointerup", { bubbles: true }));
    return res;
  });
  await page.close();
  check("nothing is drawn at rest", !out.atRest && out.restDrawn.anim === "none", out);
  check("a press starts the ring", out.whileHeld, out);
  check("which runs for as long as the hold wants",
    out.heldDrawn.anim === "lvRetryFloatHold" && out.heldDrawn.dur === "0.5s", out.heldDrawn);
  check("and the button itself still does not move", !out.moved, out);
  check("letting go early takes it away", !out.afterLetGo, out);
  check("with no menu opened", out.menusAfterShortPress === 0, out);
  check("a full hold opens the menu", out.menus === 1, out);
  check("and the ring is gone by then", !out.afterMenuOpened, out);
  check("a drag drops it too", !out.afterDrag, out);
  check("no console errors", errors.length === 0, errors);
}
