...
export function setup(ctx: Ctx, opts?: any) {
  ...
  if (
    cfg.replaceEnabled ||
    (cfg.replaceRules && String(cfg.replaceRules).trim())
  )
    pushReplaceRules();
  ...
  const isHardError = (err: any) =>
    !!err &&
    /\b(?:401|403|404|invalid api key|authentication|unauthorized|not found|does not exist|model missing)\b/i.test(
      String(err),
    );
  ...
  function buildSettingsBody(root: HTMLElement, onSaved?: () => void) {
    ...
    // debug info section (collapsible): choose what to include, review, redact, copy
    {
      ...
      dArea.style.cssText =
        "width:100%;box-sizing:border-box;font-family:monospace;font-size:12px;padding:8px;border-radius:8px;border:1px solid var(--lumiverse-border,#3a3543);background:var(--lumiverse-bg,#1a1720);color:var(--lumiverse-text,#e9e4f0);resize:vertical";
    ...
    // import / export section (collapsible, same look as the Advanced groups)
    {
      ...
      fileInput.addEventListener("change", () => {
        ...
        readFileAsText(f, (text) => {
          ...
          ioStatus =
            "Imported: " + applied.join(", ") + ". Press Save to keep it.";
