...
function applySubs(phrases: string[], subs: Array<{ from: string; to: string }>): string[] {
...
function looksLikeRefusal(text: string, cfg?: any): boolean {
  ...
  if (raw.length > maxChars) return false;         // long immersive reply, not a refusal
...
function looksLikeRefusalError(errText: string, cfg?: any): boolean {
  ...
  for (const p of splitPhrases(cfg && cfg.refusalIgnorePhrases)) if (lower.includes(p)) return false;
  for (const p of splitPhrases(cfg && cfg.refusalExtraPhrases)) if (lower.includes(p)) return true;
  if (!cfg || cfg.refusalUseBuiltins !== false) { if (REFUSAL_ERROR.test(norm)) return true; }
  return false;
}
...
export function setup(ctx: Ctx, opts?: any) {
  ...
  function pushReplaceRules() {
    try {
      if (ctx && typeof (ctx as any).sendToBackend === 'function') {
        (ctx as any).sendToBackend({ type: 'set_replace_rules', enabled: !!cfg.replaceEnabled, rulesText: String(cfg.replaceRules || ''), random: !!cfg.replaceRandom, caseSensitive: !!cfg.replaceCaseSensitive });
      }
    } catch (_) {}
  }
  ...
  function hideLiveLog() {
    if (liveLogEl && liveLogEl.parentNode) { try { liveLogEl.parentNode.removeChild(liveLogEl); } catch (_) {} }
    liveLogEl = null; liveLogBody = null;
  }
  function syncLiveLog() { if (cfg.liveLog) showLiveLog(); else hideLiveLog(); }
  ...
  function coerceKey(key: string, val: any): any {
    ...
    return f.type === 'num' ? clampField(f, val) : coerce(f.type, val, (CONFIG as any)[key]);
  }
  ...
  function onEnd(p: any) {
    ...
    if (p.error) {
      if (cfg.retryOnError) { scheduleRetry(p.chatId, 'error', p.error); return; }
      // error-retry is off, but a refusal delivered as an error should still count if the user wants refusals caught
      if (cfg.retryOnRefusal && looksLikeRefusalError(String(p.error), cfg)) { scheduleRetry(p.chatId, 'looks like an accidental refusal'); return; }
      return;
    }
  ...
  function buildDebugInfo(opts?: { settings?: boolean; buttons?: boolean; environment?: boolean; activity?: boolean }): string {
    const o = opts || {};
  ...
  function buildSettingsBody(root: HTMLElement, onSaved?: () => void) {
    ...
    // debug info section (collapsible): choose what to include, review, redact, copy
    {
      ...
      const sections: Array<{ id: 'settings' | 'buttons' | 'environment' | 'activity'; label: string }> = [
