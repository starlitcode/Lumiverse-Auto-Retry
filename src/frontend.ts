...
interface Field {
  ...
  // refusalStripThinking is off, and ignoreHardErrors is checked before
  // retryOnError rather than under it. Listing those would hide a setting that
...
function looksTruncated(
  ...
  if (((emphasis.match(/\*\*/g) || []).length) % 2 === 1) return true; // open bold run
...
function stripControlTokens(text: string): string {
  ...
  for (const mark of CONTROL_MARKS) t = t.replace(mark, " ");
  return t;
}

function stripThinking(text: string, cfg?: any): string {
  let t = String(text == null ? "" : text);
  if (cfg && cfg.refusalStripThinking === false) return stripControlTokens(t);
  ...
  t = t.replace(
    new RegExp("<\\|channel\\|>\\s*(?:" + THINK_CHANNELS + ")\\b[\\s\\S]*?(?:" + HARMONY_END + "|$)", "gi"),
    " ",
  );
...
const statedWait = (err: any): number => {
  if (!err) return 0;
...
// to its text colour, a filled button painted accent-on-text reads as a blank
// rectangle: the label is there, in the same colour as the button under it.
...
export function setup(ctx: Ctx, opts?: any) {
  ...
  const urlHolds = (id: any): boolean => {
    ...
    if (s.length < URL_ID_MIN) return false;
  ...
  const urlSaysGone = (id: any): boolean =>
    (urlNamesChats || urlSlot != null) && !addressHas(id);
  ...
  let lastReplyChat = "";
  const replyTokens = (chatId: any): number => {
    if (!lastReplyChars || String(chatId || "") !== lastReplyChat) return 0;
  ...
  let replyCountAt = 0;
  function noteReplySize(text: string, chatId: any) {
  ...
  const BUILT_IN_NOTES: Array<{ name: string; values: Record<string, any> }> = [
    {
      name: "A nudge",
      values: {
  ...
  const screenMark = (t: string): string => {
    if (!t) return "";
    let h = 0;
  ...
  function repliedWithoutUs(s: any): boolean {
    if (!s.screenAtStart) return false;
    const now = lastRenderedReply();
    if (now.length < SCREEN_REPLY_MIN) return false;
  ...
  const computeDelay = (attempt: number, rateLimited: boolean, err?: any) => {
    ...
    if (said > d) d = said + (cfg.jitter ? Math.round(Math.random() * 1000) : 0);
    return d;
  };
  ...
  function buildSettingsBody(root: HTMLElement, onSaved?: () => void) {
    ...
    function buildPresetBar(kind: string): HTMLElement {
      ...
      let undoTo: { values: Record<string, any>; pick: string } | null = null;
      ...
      // preset being loaded.
      const doLoad = (name: string, wasPick?: string): void => {
    ...
    for (const group of SCHEMA) {
      ...
      if (group.collapsed) {
        ...
        const presetBlock = (kind: string, heading: string, desc: string) => {
          const wrap = document.createElement("div");
          wrap.style.cssText = "display:flex;flex-direction:column;gap:10px";
          wrap.appendChild(hairline());
    ...
    // import / export section (collapsible, same as the schema's own)
    {
      ...
      body.appendChild(
        sectionDesc(
          "Save settings to a file or load them from one. The two lists are asked separately, because what you want out of a file and what you are willing to let a file change are different questions. An import fills in the settings above without saving, so you can review first: press Save to keep it, or close to discard.",
          false,
        ),
      );
  ...
  function retiredSwaps(): { rules: string; presets: any[] } {
    let rules = "";
    let presets: any[] = [];
    try {
      if (typeof localStorage !== "undefined") {
        const raw = JSON.parse(localStorage.getItem(STORE_KEY) || "{}");
        if (raw && typeof raw.replaceRules === "string") rules = raw.replaceRules;
        const all = JSON.parse(localStorage.getItem(PRESETS_KEY) || "{}");
        if (all && Array.isArray(all.swap)) presets = all.swap;
      }
    } catch (_) {}
    ...
    if (!rules.trim() && accountSwaps.trim()) rules = accountSwaps;
    return { rules: rules, presets: presets };
  }

  function swapsNoticeDone(): boolean {
    try {
  ...
  function buildRetiredNotice(): HTMLElement {
    ...
    body.appendChild(
      document.createTextNode(
        "It swapped words in a reply after it arrived. Rewriting a reply well is a " +
    ...
    body.appendChild(
      document.createTextNode(
        " does that job, and this one is going back to retrying. Nothing else here has changed.",
      ),
    );
    ...
    gone.addEventListener("click", () => {
      try {
        if (typeof localStorage !== "undefined") localStorage.setItem(SWAPS_SEEN_KEY, "1");
      } catch (_) {}
      try {
        box.remove();
      } catch (_) {}
  ...
  function startPicking(key: string, label: string) {
    ...
    const letGo = () => {
      if (!held) return;
      try { clearTimeout(held.timer); } catch (_) {}
      held = null;
    };
