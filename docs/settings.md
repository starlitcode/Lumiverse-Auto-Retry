...
- **Floating on/off button** puts a small button over the chat that toggles it in one tap. Drag it anywhere; it snaps to the nearest edge and stays where you leave it, and you can set its size. Changing the size grows it around where it is sitting rather than moving it, though a button against an edge does come inward far enough for the bigger size to fit. Hold it, or right-click on a computer, for a menu with **Auto Retry settings** and **Hide this button**. When the on-screen panel is set to live in the sidebar drawer, **Open the Auto Retry panel** is in there too.
