...
None of the five is required for the extension to start, and the three privileged ones may need an admin to approve them before they do anything. Without `chat_mutation` the retry side works exactly as it should and only find-and-replace does nothing. Without `ui_panels` everything works and there is no floating button. Without `chats` everything works and the per-chat switch waits to be told which chat you are in rather than asking. Without `interceptor` everything works and the refusal note is not sent.

Behind those it uses six Lumiverse APIs and nothing else: reading the messages in a chat, updating a message, asking which chat is currently open, reading and writing its own settings, and registering the interceptor above. Everything the extension does is built out of those six.
