...
**One case sends a reply to the extension's own backend, and only one.** With the panel open on the **Prompt** tab and a price filled in under **What a retry costs**, the finished reply is handed to Lumiverse's own tokeniser so the panel can say what the output half of a retry costs. Only the number comes back. The text is not stored, not written to disk, and not read for anything else, and it does not leave your Lumiverse: it is going to the server that just wrote it. Close the panel, leave the Prompt tab, or leave the prices at 0, and this does not happen at all. Without it the panel estimates the reply at four characters a token and says **roughly**.
