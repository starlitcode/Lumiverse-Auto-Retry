...
None of the five is required for the extension to start. `chat_mutation` is privileged, so depending on your Lumiverse setup it may need admin approval before it takes effect; without it the retry side works exactly as it should and only find-and-replace does nothing. Without `ui_panels` everything works and there is no floating button. Without `chats` everything works and the per-chat switch waits to be told which chat you are in rather than asking. Without `interceptor` everything works and the refusal note is not sent.
