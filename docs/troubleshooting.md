...
If the extension seems to do nothing at all, check the top of the settings panel first. A permission that was never granted is the one fault that raises no error anywhere: the events the extension listens for simply never arrive, so it sits there looking installed and working. When one is missing, the panel says which and what it costs, and the debug report lists all of them. Refusing one on purpose is a fair answer, so each note has a cross that puts it away for good; the debug report still lists every permission either way.
