...
- **Security and privacy are two documents now, because they answer two questions.** `SECURITY.md` was a long description of what the extension touches, which is not what anyone opens a security policy to find. It is now the short version people expect: how to report a problem, where to send it if you would rather not say it in public, and what to expect back. Everything about what the extension can reach, what it keeps and what it has no way to get at moved to [Privacy](docs/privacy.md), unchanged apart from being in the right place.
