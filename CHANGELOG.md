...
- **The search box's clear button was invisible on some light themes.** The same fault as above, in the one place the browser draws for us. That cross takes no colour of its own, so it is cut into a shape and filled with your theme's muted text colour instead, and that fill has to name a colour to use when the theme sets none. The one it named was for dark themes. A theme that sets the common colours but not that one filled a near-white cross on a near-white field. It now works the colour out from what the field actually paints, and leaves your theme's own colour alone wherever it already reads.
