...
- **The lowest number of tries is 1, not 0.** At 0 the give-up check passed on the very first failure, so nothing was ever retried: a retry extension sitting there reporting itself as on, with the button lit and the panel saying it was watching, and nothing happening. There are two proper ways to stop it, the master switch and the per-chat switch, and both say on screen that they are the reason. A number box is not a third one. A saved 0 is raised to 1 when it loads, so if you were using it to stop retries, use one of the switches instead.
