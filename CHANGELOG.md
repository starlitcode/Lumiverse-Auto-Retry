...
- **`docs/development.md` is gone; its four commands are back in the README.** It had grown into a page describing what every test file covers, which goes stale the moment a test is added and which anybody can read from `test/` anyway, and the rest of it repeated what [Privacy](docs/privacy.md) now says about what ships and what does not. What a contributor needs is three commands and the rule that `dist/` is committed and must be committed with `src/`.
- **Security and privacy are two documents now, because they answer two questions.** `SECURITY.md` was a long description of what the extension touches, which is not what anybody opens a security policy to find. It is now only about reporting: where to send something sensitive, where to send everything else, what to put in, what not to put in, and what to expect back. Everything that explains rather than instructs moved to [Privacy](docs/privacy.md), which is also where to look if you are auditing the extension or wondering why a scanner could not parse all of it.
...
- **The checks that run on a pull request pin the actions they use to a commit rather than to a tag.** A tag is movable: whoever owns an action can point `v4` at anything later, and every run after that would fetch it with no diff here to review. The checkout step also keeps no credentials in the build environment, since nothing in the checks pushes anything. Neither was reachable by anyone using the extension, and both were fair to raise.
