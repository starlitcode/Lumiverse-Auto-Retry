...
- **Switching off in one chat was missing from the floating button's menu until the first reply.** The entry was only drawn once a chat had been seen, and that only happens on a generation, so on a fresh page load it was not there at all. Rather than fix the gate it has been taken out of that menu for good: the button sits over your chat and its menu is opened to reach the settings or to move the button, so a per-chat switch among those entries was one more thing to read past every time. It is where it always was as well, in the settings panel under **Basics** on the **This chat** row, along with the line at the top of the panel that says when you are in a chat that is switched off. Nothing about the feature itself changed.
