...
async function countTokens(text, context, userId) {
    try {
        if (!text || !spindle.tokens || typeof spindle.tokens.countText !== 'function')
            return null;
...
function thinkSpans(text) {
    ...
    for (let i = 1; i < found.length; i++) {
        ...
        if (found[i][0] <= last[1])
            last[1] = Math.max(last[1], found[i][1]);
        else
            out.push(found[i]);
    }
    return out;
...
function applyRulesVisible(text, seen) {
    ...
    for (const [s, e] of spans) {
        if (s > at)
            out += applyRules(src.slice(at, s), seen);
        out += src.slice(s, e);
        at = e;
    }
    if (at < src.length)
        out += applyRules(src.slice(at), seen);
    return out;
}
...
function scheduleSwap(chatId, messageId, sawEdit, userId) {
    ...
    if (!p) {
        p = { chatId: String(chatId), capAt: Date.now() + MAX_WAIT_MS, timer: null };
...
function onForeignEdit(chatId, messageId, content, userId) {
    ...
    try {
        ...
        if (payload.type === 'load_settings') {
            ...
            if (settings && typeof settings === 'object')
                applyReplaceFromSettings(settings);
        ...
        if (payload.type === 'get_permissions') {
            ...
            try {
                ...
                if (perms && typeof perms.getGranted === 'function') {
                    ...
                    if (Array.isArray(live) && live.every((x) => typeof x === 'string'))
...
spindle.on('GENERATION_ENDED', async (p) => {
    try {
        ...
        if (!messageId)
            return;
        if (waitForOtherEdits) {
            scheduleSwap(chatId, messageId, false, p.userId);
            return;
        }
...
function grantedMap(e) {
    ...
    const usable = Array.isArray(all) && all.every((x) => typeof x === 'string');
    const fromAll = (name) => usable ? all.indexOf(name) >= 0 : null;
    const out = {};
