...
export function setup(ctx, opts) {
    ...
    function pageSlept(due) {
        ...
        if (Date.now() - saidPageSlept > 1000) {
            ...
            log("this page was not running for part of the wait, so the reply gets the " +
    ...
    function abortAndRetry(chatId, reason) {
        ...
        if (repliedWithoutUs(s)) {
            ...
            s.live = false;
            s.screenAtStart = "";
            log("a reply arrived on this page without the tab being told, so it was " +
    ...
    function whichChatIsOpen(forChat) {
            ...
            for (const root of roots) {
                ...
                while (node) {
                    ...
                    if (!skip && parent.closest) {
                        try {
                            skip = !!parent.closest("[data-ar-ui],[contenteditable='true'],[role='dialog'],[role='menu'],[role='tooltip']");
                        }
                        catch (__) { }
                    }
                    if (!skip)
    ...
    function rememberSwap(before, after) {
        ...
        while (swapUndos.length > SWAP_UNDO_CAP)
            swapUndos.shift();
    }
    ...
    const EDIT_BOX_SELECTOR = 'textarea[name="message-edit-content"],textarea[aria-label="Message content"]';
    function repairEditBox() {
        if (!swapUndos.length || typeof document === "undefined")
            return false;
        let fixed = false;
        let boxes = [];
        try {
    ...
    function onDocFocusIn(e) {
        try {
            if (!swapUndos.length)
                return;
            const t = e && e.target;
            if (!t || !t.matches || !t.matches(EDIT_BOX_SELECTOR))
                return;
            scheduleEditRepair();
        }
        catch (_) { }
            ...
            const offRep = ctx.onBackendMessage(async (msg) => {
                try {
                    ...
                    if (msg.type === "nothing_left_to_swap") {
                        log("another extension rewrote a reply while the swap waited on it, " +
                    ...
                    if (msg.waitsEnded > 0)
                        log("stopped waiting for another extension to finish editing, on " +
