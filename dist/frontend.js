...
const SETUP_PRESETS = [
    ...
    },
    {
        id: "slow",
        label: "Slow model",
        blurb: "For a local model, a large one, or anything that takes its time before the first word. Waits far longer before deciding a reply has stalled, so a slow but healthy reply is never thrown away.",
        values: {
    ...
    },
    {
        ...
        blurb: "For a free or shared tier that meters per minute. Waits much longer when the server says it is busy, backs off harder, and gives up sooner rather than spending your quota on retries.",
        values: {
    ...
    },
    {
        ...
        blurb: "Retries a reply that failed or came back empty, and nothing else. Nothing reads your replies to judge them, so a reply is never re-rolled for looking cut off or for looking like a refusal.",
        values: {
...
function looksTruncated(text, retryOnNoPunct, cfg) {
    ...
    if (retryOnNoPunct && !endsOnPunctuation(t))
        return true;
    return false;
}
            ...
            for (const preset of SETUP_PRESETS) {
                ...
                b.addEventListener("click", () => {
                    ...
                    for (const key of TUNING_KEYS) {
                        const want = key in preset.values ? preset.values[key] : CONFIG[key];
                        const f = fieldByKey[key];
