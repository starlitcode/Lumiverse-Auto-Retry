...
const CONFIG = {
    ...
    regenerateSelector: '[data-composer-action="regen"] button, ' +
    ...
    },
    {
        ...
        fields: [
            ...
            },
            {
                ...
                type: "bool",
                hint: "On by default. A line inside quotation marks is a character speaking, so it is not counted as the model refusing. Your own phrases are counted either way.",
            },
            {
...
const HTML_KNOWN = ("html|head|body|div|span|p|a|b|i|u|s|em|strong|small|mark|sub|sup|code|pre|kbd|samp|var|" +
...
function endsOnABlock(shown, visible) {
    ...
    if (lastLine[0] !== "|" && (lastLine.match(/\|/g) || []).length >= 2)
        return true;
    ...
    const listItem = (l) => /^(?:[-*+]\s+|\d+[.)]\s+)\S/.test(l);
    ...
    const danglingWord = /\b(?:a|an|and|are|as|at|be|been|being|but|by|can|could|for|from|had|has|have|he|her|his|i|in|into|is|it|its|may|might|must|my|no|not|of|on|onto|or|our|over|she|should|that|the|their|they|to|under|was|we|were|which|who|will|with|would|you|your)$/i;
    if (lines.length >= 2 &&
...
function thinkOpenedNeverClosed(raw, cfg) {
...
function looksTruncated(text, retryOnNoPunct, cfg) {
    ...
    if (thinkOpenedNeverClosed(raw, cfg))
        return true;
    ...
    if ((prose.match(/"/g) || []).length % 2 === 1) {
        ...
        if ((withoutInches.match(/"/g) || []).length % 2 === 1)
            return true;
    }
...
function spamVerdict(content, reasoning, cfg, look) {
    ...
    if (thinking && spamChar(inline))
        return "thinking";
    if (reply && spamChar(shown))
        return "reply";
    return "";
}
...
function carrySpamSwitch(saved) {
...
const DIALOGUE_TAG = /^[,.!?"'\u2014\u2013 ]{0,4}(?:(?:the|a|an|his|her|their|my|our|your) )?(?:he|she|they|it|i|[A-Za-z][a-z]+)\s+(?:said|says|replied|replies|answered|answers|added|adds|muttered|mutters|murmured|whispered|whispers|told|asked|asks|continued|continues|snapped|snaps|sighed|sighs|declared|announced|insisted|thought|thinks|decided|decides|wrote|writes|breathed|growled|hissed|warned|promised|admitted|agreed|repeated|offered|called|calls)\b/;
...
const CONTROL_MARKS = [
    ...
    new RegExp("[ \\t]*(?:<\\|(?:start|turn|im_start)\\|?>|<start_of_turn>)(?:[ \\t]*(?:" +
        TURN_ROLES +
        ")\\b)?[ \\t]*", "gi"),
...
function stripThinking(text, cfg) {
    ...
    // with both ends is left to the pairs below.
    {
        ...
        if (close) {
            const open = new RegExp("<(?:" + alt + ")(?:\\s[^>]*)?>|\\[(?:" + alt + ")(?:\\s[^\\]]*)?\\]|<\\|(?:" + alt + ")\\|?>", "i");
            if (!open.test(t.slice(0, close.index)))
                t = " " + t.slice(close.index + close[0].length);
        }
    }
...
function spanIsQuoted(text, start) {
    const QUOTES = "\"\u201c\u201d\u00ab\u00bb";
    let marks = 0;
...
function isHardError(err, cfg) {
    ...
    for (const p of splitPhrases(cfg && cfg.hardErrorPhrases))
        if (lower.includes(p))
            return true;
    return /\b(?:400|401|402|403|404|405|406|411|413|415|422|invalid api key|authentication|unauthorized|not found|does not exist|model missing|insufficient balance|permission|forbidden|not allowed)\b/i.test(text);
}
...
const PAGE_FALLBACK = [20, 16, 30, 1]; // Lumiverse is dark; last resort only
...
let repairs = [];
function keepReadable(el, kind, min) {
    if (!el)
        return;
    ...
    for (const r of repairs)
        if (r.el === el && r.kind === kind && r.min === min)
            return;
...
function stripInk(root) {
    try {
        const all = [root].concat(root.querySelectorAll ? Array.prototype.slice.call(root.querySelectorAll("[data-ar-ink],[data-ar-edge]")) : []);
        for (const el of all) {
            if (!el || !el.getAttribute)
                continue;
    ...
    for (const r of liveRepairs()) {
        try {
            if (r.kind === "scheme") {
                ...
                if (bg)
                    r.el.style.colorScheme = relLuminance(bg) < 0.5 ? "dark" : "light";
                continue;
            }
            stripInk(r.el);
            if (r.kind === "tree")
                sweepTree(r.el, r.min);
            else if (r.kind === "ink")
                fixContrast(r.el, r.min);
            else
                fixEdge(r.el, r.min);
        }
        catch (_) { }
...
function releaseScroll(held) {
    const put = () => {
        for (const one of held) {
            try {
                if (one.node && one.node.scrollTop !== one.at)
...
const MARK_SLASH = '<line class="lv-ar-slash" stroke-width="2" x1="3.5" y1="20.5" x2="20.5" y2="3.5"/>';
...
function holdRingSvg() {
    return ('<svg class="lv-ar-hold" viewBox="0 0 100 100" aria-hidden="true" focusable="false">' +
...
export function setup(ctx, opts) {
    ...
    function refreshFromAccount() {
        if (Date.now() - accountAskedAt < STALE_AFTER_MS)
            return;
        loadFromAccount();
    ...
    const permHidden = new Set();
    const permIsHidden = (name) => permHidden.has(name);
    ...
    try {
        if (typeof localStorage !== "undefined") {
            ...
            if (raw && typeof raw === "object") {
                ...
                const p = raw.panel;
                if (p && asNumber(p.x) !== null && asNumber(p.y) !== null &&
    ...
    let promptView = layout.promptView || "rendered";
    let paintTabs = null;
    let focusTab = null;
    ...
    function rememberReplaced(chatId, text, reason) {
        ...
        while (replaced.size > REPLACED_MAX)
            replaced.delete(replaced.keys().next().value);
    }
    ...
    function noteReplySize(text, chatId) {
        ...
        try {
            if (ctx && typeof ctx.sendToBackend === "function")
                ctx.sendToBackend({ type: "count_reply", at: at, text: text });
        }
        catch (_) { }
    }
    ...
    const sayTokens = (chars) => lastPromptTokens
    ...
    function renderStatsView() {
        ...
        function barBlock(title, counts, order) {
    ...
    function replacedHere() {
        return replaced.get(chatSlot(lastChatId)) || null;
    }
    function replacedAsText() {
    ...
    function buildPanelParts(draggable) {
        ...
        const statsAsText = () => {
            ...
            const total = stats.good + stats.retries;
            if (total)
                lines.push("One reply in: " +
                    (stats.retries
                        ? String(Math.max(1, Math.round(total / stats.retries))) + " needed a retry"
                        : "none needed a retry"));
            if (cfg.pauseWhenFailing && Date.now() < pausedUntil)
                lines.push("Paused after repeated failures. " + sayTime(pausedUntil - Date.now()) +
                    " left, or until a reply comes back fine.");
            const names = Object.keys(stats.reasons).sort((a, b) => stats.reasons[b] - stats.reasons[a]);
            if (!names.length) {
        ...
        copyBtn.addEventListener("click", async () => {
            ...
            const ok = await copyText(liveTab === "prompt"
                ? promptAsText()
                : liveTab === "stats"
                    ? statsAsText()
                    : liveTab === "replaced"
        ...
        clearBtn.addEventListener("click", () => {
            if (liveTab === "prompt") {
                ...
                stats.lastNoteSkip = "";
                for (const k of Object.keys(stats.reasons))
                    delete stats.reasons[k];
    ...
    function showLiveLog() {
        ...
        el.__stopStatus = parts.stopStatus;
        el.appendChild(head);
    ...
    function showDrawerPanel() {
        ...
        try {
            ...
            paintBadge = () => {
                ...
                drawerBadge = want;
                try {
                    drawerTab && drawerTab.setBadge && drawerTab.setBadge(want);
                }
                catch (_) { }
            };
            paintBadge();
    ...
    const focusQuietly = (el) => {
        ...
        const clear = () => {
            try {
                el.removeAttribute(QUIET_ATTR);
            }
            catch (_) { }
            try {
                el.removeEventListener("keydown", clear, true);
            }
            catch (_) { }
            try {
    ...
    function paintFloat() {
        ...
        if (floatEl.getAttribute("data-ar-glyph") !== String(glyph)) {
            ...
            if (box)
                box.innerHTML = markSvgLive(glyph);
        }
    ...
    function showFloat(at) {
        ...
        holdMoveWatch = (e) => {
            if (!pressFrom || !e)
                return;
            if (Math.abs(e.clientX - pressFrom.x) > HOLD_SLOP ||
    ...
    function watchTheme() {
        ...
        try {
            ...
            if (document.body)
                eye.observe(document.body, marks);
            if (document.head)
    ...
    function swipeFirstOnce(parsed) {
        try {
            if (typeof localStorage === "undefined")
                return;
            if (localStorage.getItem(SWIPE_FIRST_KEY))
                return;
    ...
    function movedForMe() {
        try {
            if (typeof localStorage === "undefined")
                return [];
            if (String(localStorage.getItem(MOVED_SEEN_KEY) || "") === MOVED_MARK)
                return [];
        }
        catch (_) {
            return [];
        }
        return MOVED_DEFAULTS.filter((m) => Number(cfg[m.key]) === m.was);
    }
    function markMovedSeen() {
        try {
            if (typeof localStorage !== "undefined")
                localStorage.setItem(MOVED_SEEN_KEY, MOVED_MARK);
        }
        catch (_) { }
    }
    ...
    function markBuiltInSeen() {
        try {
            if (typeof localStorage !== "undefined")
                localStorage.setItem(BUILT_IN_SEEN_KEY, builtInMark);
        }
        catch (_) { }
    }
    ...
    try {
        if (typeof localStorage !== "undefined") {
            ...
            if (Array.isArray(raw))
                chatsOff = raw.map(String).slice(-CHATS_OFF_CAP);
        }
    }
    catch (_) { /* no storage: nothing is off, which is the harmless way round */ }
    const chatIsOff = (chatId) => chatId != null && chatsOff.indexOf(String(chatId)) >= 0;
    ...
    function tellBackendChatsOff() {
        try {
            if (ctx && typeof ctx.sendToBackend === "function")
                ctx.sendToBackend({ type: "set_chats_off", chats: chatsOff.slice() });
        }
        catch (_) { }
    }
    ...
    // place that says so.
    const retryingHere = (chatId) => cfg.enabled !== false && !chatIsOff(chatId);
    ...
    const chatSlot = (chatId) => chatId == null || chatId === "" ? NO_CHAT : String(chatId);
    const chatOf = (p) => chatSlot(p && p.chatId);
    ...
    function chatStatus(s) {
        ...
        if (s.live && s.sawReasoning)
            return { text: "Model is thinking" + soFar, busy: true };
        ...
        if (s.live)
            return {
                text: (!sawStreaming && sawWholeReplyAtOnce
    ...
    function addTicker(f) {
        tickers.add(f);
        retick();
    }
    function removeTicker(f) {
        tickers.delete(f);
        retick();
    }
    ...
    let saidPageSlept = 0;
    function pageSlept(due) {
        if (Date.now() - due <= WATCHDOG_SLACK_MS)
            return false;
        if (Date.now() - saidPageSlept > 1000) {
            saidPageSlept = Date.now();
            log("this tab was asleep for part of the wait, so the reply starts its " +
    ...
    function armWatchdog(s, field, delay, fire) {
        if (s[field])
            clearTimeout(s[field]);
    ...
    const clickable = (el) => {
        if (!el)
            return false;
        try {
            if (el.closest && el.closest(OWN_UI))
                return false;
            if (el.disabled)
                return false;
    ...
    const find = (selector, builtIn) => {
        ...
        if (builtIn)
            for (const extra of splitSelectorList(builtIn))
                if (parts.indexOf(extra) < 0)
                    parts.push(extra);
        if (typeof document === "undefined")
            return null;
    ...
    const fireRetry = () => {
        ...
        if (picked && picked.via !== (cfg.retryByNewReroll ? "swipe" : "regenerate"))
            log("no " + (cfg.retryByNewReroll ? "next / swipe" : "regenerate") +
    ...
    function findNewConfirm(before) {
        ...
        for (const el of Array.prototype.slice.call(list)) {
            ...
            try {
                ...
                if (el.closest && el.closest("[data-ar-ui]"))
                    continue;
            }
            catch (_) { }
            fresh.push(el);
    ...
    function abortAndRetry(chatId, reason) {
        ...
        if (repliedWithoutUs(s)) {
            ...
            s.live = false;
            s.screenAtStart = "";
            log("the reply is on the page but this tab never heard it arrive, so it " +
    ...
    function askActiveChat(forChat, answer) {
        const reply = (r) => {
            if (!answer)
                return;
            const send = answer;
    ...
    const stillWondering = () => lastChatId == null && (idInUrl() != null || urlSlot == null);
    ...
    function onEnd(p) {
        ...
        if (genId) {
            ...
            while (s.judged.size > IGNORE_MAX)
                s.judged.delete(s.judged.values().next().value);
        }
        ...
        const thought = typeof p.reasoning === "string" && p.reasoning.trim()
            ? p.reasoning
            : String(s.thinkBuf || "");
        s.thinkBuf = "";
        paintNow();
    ...
    function setToastText(msg) {
        ...
        if (!t || !t.__words || t.style.opacity !== "1")
            return;
    ...
    // countdown at all.
    function startToastCountdown(s) {
        stopToastCountdown();
    ...
    function buildSettingsBody(root, onSaved) {
        ...
        const applyAndSave = () => {
            ...
            for (const sync of driftSyncers)
                sync();
            if (onSaved)
                onSaved();
            return storedHere;
        ...
        function buildPresetBar(kind) {
            ...
            const isBuiltIn = (name) => builtIns().some((p) => p.name === name);
        ...
        applyDeps = () => {
            ...
            for (const d of depRows) {
                const on = d.groups.every((g) => g.some((k) => !!cfg[k]));
                showsNow(d.row, on);
            }
        ...
        for (const group of SCHEMA) {
            ...
            const addRow = (row, f) => {
                ...
                if (f.needs && f.needs.length)
                    own.push(f.needs);
                if (f.needsAll && f.needsAll.length)
                    for (const k of f.needsAll)
                        own.push([k]);
                if (own.length)
                    depRows.push({ row: row, groups: own });
                const groups = own.slice();
                if (group.needs && group.needs.length)
        ...
        // import / export section (collapsible, same as the schema's own)
        {
            ...
            // back, and gone the moment there is not.
            {
                ...
                if ((count || held.presets.length) && swapsOffered(todayHere())) {
                    ...
                    get.addEventListener("click", () => {
                        const ok = downloadText("auto-retry-word-swaps.json", JSON.stringify({
            ...
            fileInput.addEventListener("change", () => {
                ...
                const which = (at) => picked.length === 1 ? "that file" : picked[at].name;
    ...
    function buildMovedNotice() {
        ...
        take.addEventListener("click", () => {
            for (const m of moved) {
                ...
                if (fieldSetters[m.key])
                    fieldSetters[m.key](cfg[m.key]);
            }
            markMovedSeen();
            saveSaved();
            saveToAccount();
            ...
            const offRep = ctx.onBackendMessage(async (msg) => {
                try {
                    ...
                    if (msg.type === "prompt_tokens") {
                        if (!lastPrompt || (msg.at && lastPrompt.at && msg.at !== lastPrompt.at))
                            return;
