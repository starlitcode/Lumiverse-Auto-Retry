...
export function setup(ctx, opts) {
    ...
    if (cfg.replaceEnabled || (cfg.replaceRules && String(cfg.replaceRules).trim()))
        pushReplaceRules();
    ...
    function showLiveLog() {
        ...
        const onUp = (e) => { if (dragging) { dragging = false; try { head.releasePointerCapture(e.pointerId); } catch (_) { } } };
        ...
        const rzUp = (e) => { if (rz) { rz = false; try { grip.releasePointerCapture(e.pointerId); } catch (_) { } } };
